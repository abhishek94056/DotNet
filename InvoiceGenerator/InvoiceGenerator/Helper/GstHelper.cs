namespace InvoiceGenerator.Helper
{
    ...
    {
        // ── Per-item amounts (uses item's own GST%) ──
        public static (decimal taxableAmount, decimal gstAmount, decimal total)
            CalcItemAmounts(decimal rate, decimal qty, decimal gstPercent)
        {
            decimal taxable = rate * qty;
            decimal gstAmount = taxable * (gstPercent / 100m);
            return (taxable, gstAmount, taxable + gstAmount);
        }

        // ── Invoice-level GST split (CGST/SGST or IGST) ──
        // taxableValue here = sum of (Rate × Qty) before GST
        // gstAmount = sum of all per-item GST amounts
        public static (decimal cgst, decimal sgst, decimal igst, decimal total)
            SplitGst(decimal taxableValue, decimal totalGstAmount, int stateCode)
        {
            ...
            return (cgst, sgst, igst, taxableValue + totalGstAmount);
        }

        // ── Keep old Calculate for backward compat ──
        public static (decimal cgst, decimal sgst, decimal igst, decimal total)
